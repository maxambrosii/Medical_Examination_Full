...
(
	...
	gender bit, -- 0 - Мужчина, 1 - Женщина
	IDNP Char(13),
	Strahovka Char(15),
...
--=========================#1_Вывод пациентов по номеру участка=========================--
...
print 'Информация о пациенте по введеному участку : ' +  CAST(@NumOfUchastok AS NVARCHAR(5))
Select *
	From @GetPacient
--============================#2_Самая частая группа лекарств за день-===================--
Select * From Lekarstvo
go
...
--============================#3_Количество пациентов за день у доктора-===================--
Create function dbo.uf_GetPatientsByDate(@date date, @FIO_Doctor AllNames)
	returns int as
	begin
		Declare @CountOfPatient int
		Select @CountOfPatient = (Select count(dbo.Pacient.Cod_Pacient) as 'Кол-во пациентов за день'
			From dbo.Pacient
...
Print 'Количество пациентов за день у ' + @FIO_Doctor + ' = ' + CAST(dbo.uf_GetPatientsByDate(@userdate, @FIO_Doctor) AS NVARCHAR(5))
--Запрос работает правильно. Нужно добавить людей в БД. Для проверки дату поставь <= вместо =
--==============================#4_Самый частый диагноз за день=========================--	
...
Set @userdate = '2018-10-25'
print 'Самый частый диагноз за день это ' + dbo.uf_GetFrequentDiagnosis(@userdate)
--==============================#5_Сколько детей, пожилых обратилось в полеклинику=====--
...
Set @agetype = 2
Print 'Количество человек за день ' + ' = ' + CAST(dbo.uf_GetCountPacientByCertainAge(@agetype) AS NVARCHAR(5))
--======================================Multi Statement================================================--\
-- Количество пациентов у каждого доктора за определенную дату
...
Select * From dbo.uf_GetPatientByDate('2018-10-25')
go
...
--=========================================================================================--
--Меняем время приема по дате и имени Доктора. + Кол-во добавленного времени
...
--=========================================================================================--
--Вывести кол-во пациентов, у докторов с определенны диагнозом
--В функцию вводим название диагноза
drop view DoctorPriemView
drop function dbo.uf_GetDoctorWithDiagnoz
Create view DoctorPriemView as
	Select dbo.Doctor.FIO_Doctor, dbo.Priem.Cod_Diagnoz
		From dbo.Doctor inner Join dbo.Priem
			on dbo.Doctor.Cod_Doctor = dbo.Priem.Cod_Doctor

Create function dbo.uf_GetDoctorWithDiagnoz(@diagnoz varchar(100))
	returns table as
	return (
			Select dbo.DoctorPriemView.FIO_Doctor, count(dbo.DoctorPriemView.Cod_Diagnoz) as 'Количество'
				From dbo.DoctorPriemView
					where dbo.DoctorPriemView.Cod_Diagnoz in (
						Select dbo.DocDiagnoz.Cod_Diagnoz
							From dbo.DocDiagnoz
								where dbo.DocDiagnoz.Diagnoz = @diagnoz
					)
					group by dbo.DoctorPriemView.FIO_Doctor
			)
Select * From DoctorPriemView
Select * From dbo.uf_GetDoctorWithDiagnoz('Nasmork')
...
drop Trigger DeleteFromLekarstvo
Create Trigger DeleteFromLekarstvo
	On dbo.Lekarstvo
		Instead of Delete as
			begin
				Delete From dbo.Lechenie
					where dbo.Lechenie.Cod_Lekarstva in (
						Select deleted.Cod_Lekarstva
							From deleted
					)
				Delete From Lekarstvo
					where dbo.Lekarstvo.Cod_Lekarstva in (
						Select deleted.Cod_Lekarstva
							From deleted
					)
			end
			
Delete From dbo.Lekarstvo
	where(dbo.Lekarstvo.Cod_Lekarstva = 4001)
Select * From Lekarstvo	
Select * From Lechenie
